...
def random_herm_sparse_spectrum(N, seed, density, verb=False, sparse=True):
  np.random.seed(seed)
  num_nonzero_elems = int(np.ceil(N**2 * density / 2))
  elem_idxs = np.random.randint(0, N, size=(num_nonzero_elems, 2))
  elems = np.random.rand(num_nonzero_elems) + np.random.rand(num_nonzero_elems) * 1j
  A = sp.csc_matrix((elems, (elem_idxs[:, 0], elem_idxs[:, 1])), shape=(N, N))
  A = A + A.conj().T
  
  if sparse: 
    eigenvalues, eigenvectors = sp.linalg.eigsh(A, k = N-2)  # 'eigsh' is used for Hermitian sparse matrices
  else:
    A = A.toarray()  # Convert to dense
...
def compute_norm_spacing(N, seed, mode = 'herm', density = 0.1, trim = False):
  if mode == 'herm':
    eigval, _ = random_herm_spectrum(N, seed)
  elif mode == 'diag':
    eigval, _ = random_diag_spectrum(N, seed)
  elif mode == 'sparse':
    eigval, _ = random_herm_sparse_spectrum(N, seed, density)
    None
...
def compute_spacing_distr(N, seed, N_matrices, mode, density=0.1, trim = False):
  spacing_distr = []
  
  for i in range(N_matrices):
    seed = seed + N * i
    spacings = compute_norm_spacing(N, seed, mode=mode, density=density, trim=trim)
